...
-- Battery full line
	Def.Sprite{
		Texture=THEME:GetPathB("","ScreenGameplay decorations/" .. stream ),
		...
		BeginCommand=function(s,p)
			...
			if glifemeter:GetTotalLives() == 1 then
				s:Load(THEME:GetPathB("","ScreenGameplay decorations/danger"));
				s:setsize(640,48);
			end
		end,
		LifeChangedMessageCommand=function(self,params)
			...
			if params.LivesLeft == 1 then
				self:Load(THEME:GetPathB("","ScreenGameplay decorations/danger"));
				self:setsize(640,48);
			else
				self:Load(THEME:GetPathB("","ScreenGameplay decorations/normal"));
				self:setsize(640,48);
			end;
		end;
	};

...
local function MakeRow(rownames, idx)
	...
	return Def.ActorFrame{
		...
		LoadFont("_avenirnext lt pro bold 25px")..{
			...
			GainFocusCommand=function(s) s:visible(false) end,
			LoseFocusCommand=function(s) s:visible(true) end,
		};
		Def.Quad{ OnCommand=function(s) s:zoomto(160,26):x(360):MaskSource() end; };
		Def.Quad{ OnCommand=function(s) s:zoomto(160,26):x(40):MaskSource() end; };
		Def.ActorFrame{
			...
			[p"MenuLeft%MessageCommand"]=function(s)
				...
				if screen then
					local choice = screen:GetOptionRow(idx-1):GetChoiceInRowWithFocus(pn);
					if choice == 0 then
						s:stoptweening():x(-20):decelerate(0.2):x(0)
					else
						s:stoptweening():x(-150):decelerate(0.3):x(0)
					end
				end
			end,
			[p"MenuRight%MessageCommand"]=function(s)
				local screen = SCREENMAN:GetTopScreen()
				if screen then
					local numchoices = screen:GetOptionRow(idx-1):GetNumChoices()
					local choice = screen:GetOptionRow(idx-1):GetChoiceInRowWithFocus(pn);
					if choice == numchoices-1 then
						s:stoptweening():x(20):decelerate(0.3):x(0)
					else
						s:stoptweening():x(0):decelerate(0.2):x(-150)
					end
				end
			end,
			LoadFont("_avenirnext lt pro bold 25px")..{
				...
				[p"MenuLeft%MessageCommand"]=function(s) s:sleep(0.18):queuecommand("Set") end,
				[p"MenuRight%MessageCommand"]=function(s) s:sleep(0.18):queuecommand("Set") end,
				CurrentSongChangedMessageCommand=function(s) s:queuecommand("Set") end,
			};
			LoadFont("_avenirnext lt pro bold 25px")..{
				InitCommand=function(s) s:x(350):uppercase(true):zoom(0.9):maxwidth(150) end,
				OnCommand=function(s) s:queuecommand("Set") end,
				SetCommand=function(self)
					if IsExitRow() then return end;
					local screen = SCREENMAN:GetTopScreen();
			...
			LoadActor("arrow")..{
				...
				[p"MenuLeft%MessageCommand"]=function(s)
					...
					if screen then
						local choice = screen:GetOptionRow(idx-1):GetChoiceInRowWithFocus(pn);
						local oldchoice
						if choice > 0 and choice ~= 0 then
							s:visible(true):finishtweening():diffuse(color("#8080ff")):sleep(0.3):linear(0.4):diffuse(color("1,1,1,1"))
							s:sleep(1)
						else
							s:visible(false)
						end
...
local t = Def.ActorFrame{
	...
	OffCommand=function(s) s:sleep(0.2):accelerate(0.2):addy(-SCREEN_HEIGHT) end,
	Def.ActorFrame{
		LoadActor("Backer");
		Def.ActorFrame{

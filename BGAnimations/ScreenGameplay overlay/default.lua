...
for pn in ivalues(GAMESTATE:GetHumanPlayers()) do
	...
	t[#t+1] = Def.Sprite{
		...
		InitCommand=function(s) s:visible(false):xy(pn==PLAYER_1 and ScreenGameplay_P1X() or ScreenGameplay_P2X(),_screen.cy) end,
		BobCommand=function(s) s:bob():effectmagnitude(0,10,0):effectperiod(1) end,
		HealthStateChangedMessageCommand= function(self, param)
			if param.PlayerNumber == pn then
			if param.HealthState == 'HealthState_Dead' and GAMESTATE:PlayerIsUsingModifier(pn,"battery") then
